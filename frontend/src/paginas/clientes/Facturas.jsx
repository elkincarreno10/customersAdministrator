...
const FacturasCliente = () => {
    ...
    const { facturas, setFacturas, setFacturaEditar, eliminarFactura, alerta, cargando } = useClientes()
    ...
    const handleEditarFactura = factura => {
        ...
        navigate('/editar-factura-cliente')
    }
    ...
    const handleEditarEstado = async facturaActualizada => {
        ...
        try {
            ...
            const cambio = await clienteAxios(`/api/clientes/cambiar-estado/${facturaActualizada.id}`, config)
  ...
  return (
    <div className="bg-gray-50 flex-1 flex flex-col gap-3 py-10 px-10 lg:px-24">
        ...
        <div className="flex justify-between">
            <Link to='/crear-factura-cliente' className="bg-blue-400 hover:bg-blue-600 text-white rounded-lg p-2 text-center w-32 mb-2">Nueva Factura</Link>
            <Link to='/clientes' className="bg-blue-400 hover:bg-blue-600 text-white rounded-lg p-2 mb-2">Volver</Link>
        </div>
