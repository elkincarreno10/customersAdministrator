...
const EditarFacturaCampesino = () => {

  const { campesinoFacturas, actualizarFactura, facturaEditar, alerta, setAlerta } = useCampesinos()

  const [ descripcion, setDescripcion ] = useState(facturaEditar.descripcion)
  const [ saldo, setSaldo ] = useState(facturaEditar.saldo)
  const [ fecha, setFecha ] = useState(facturaEditar.fecha.split('T')[0])
  ...
  const handleActualizarFactura = e => {
    ...
    actualizarFactura({ descripcion, saldo, fecha, campesinoId: campesinoFacturas })
  }
  ...
  return (
    <div className="flex-1 max-w-2xl mx-auto mt-16 p-5">
      <div className="flex justify-between items-center">
        <h1 className="font-bold text-xl mb-6">Editar Factura</h1>
        <Link to={`/facturas-campesino/${clienteFacturas}`} className="bg-blue-400 hover:bg-blue-600 text-white rounded-lg p-2">Volver</Link>
      </div>
      ...
      >
        ...
        <div className="flex gap-1 items-center">
            ...
            <input 
                className="p-1 pl-2 rounded-lg flex-1 ml-1"
                type="number"
                placeholder="Ingresa el nuevo valor de la factura"
                id="saldo"
                name='saldo'
                onChange={e => setSaldo(+e.target.value)}
            />
        </div>
        <p>Saldo Anterior: $ {formatearPrecio(facturaEditar.saldo)}</p>
        <div className="flex gap-1 items-center">
...
export default EditarFacturaCampesino